"""Train the cosine bi-encoder baseline as the fair comparison target."""
...
from training.cosine_baseline import CosineBaselineConfig, train_cosine_baseline
...
def main() -> int:
    ...
    ap.add_argument("--log-path", type=Path, default=Path("logs/cosine_baseline.log"))
    ...
    config = CosineBaselineConfig(
        **{k: v for k, v in cfg_dict.items() if k in CosineBaselineConfig.__dataclass_fields__}
    )
    ...
    train_cosine_baseline(dataset, config, args.checkpoint_dir, args.log_path)
    return 0
